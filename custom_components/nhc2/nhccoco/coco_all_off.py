...
from .const import KEY_ALL_OFF_ACTIVE, VALUE_TRIGGERED, VALUE_TRUE, KEY_BASICSTATE
from .helpers import extract_property_value_from_device

_LOGGER = logging.getLogger(__name__)


class CoCoAllOff(CoCoEntity):
    ...
    def update_dev(self, dev, callback_container=None):
        _LOGGER.info(f'update_dev: {dev}')
        has_changed = super().update_dev(dev, callback_container)
        status_value = extract_property_value_from_device(dev, KEY_ALL_OFF_ACTIVE)
